...
// Copyright (c) 2014 The Madoocoin developers
...
#if defined(NDEBUG)
# error "Madoocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x23c5541b417a759214e04366a1d0d4da7eab33ff0e9e780a682d0109e84324ea");
uint256 hashGenesisBlockTestNet("0x3d214d299b327ea5e3d498bfc2ad73f6e5082d70db7116f986d180e30301108d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 18); // Madoocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Madoocoin Signed Message:\n";
...
static CBigNum bnProofOfStakeReset(~uint256(0) >> 20); // 1
int64 nReserveBalance = 0;
unsigned int nStakeMinAge = 1 * 60 * 60; // 8 hours
unsigned int nStakeMaxAge = 14 * 24 *  60 * 60; // 45 days
...
{
    // Madoocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
const int64 nTargetSpacing = 60; // Madoocoin: 1 minute block
static const int64 nTargetTimespan = 24 * 60 * 60; // Madoocoin: Not used
...
void ThreadScriptCheck() {
    RenameThread("madoocoin-scriptch");
...
{
    // Madoocoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime = 1546506552;
        block.nBits = 0x1e0ffff0;
        block.nNonce = 222927360;

        if (fTestNet)
        {
            txNew.nTime = block.nTime = 1546506522;
            block.nNonce = 3635302;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x2ea399bf997b6f13ce649be8b8ead622696ce7e23a5255493685725613cccc79"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x3d, 0xd5, 0x5f, 0xf9 }; // Madoocoin: increase each by adding 2 to bitcoin's value.
